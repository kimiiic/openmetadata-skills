...
```bash
uv run discover-data --intent '{"entity_type":"table","constraints":[{"field":"columns.name","operator":"match","value":"email"}]}' --limit 10
```

The alias `disover-data` also works.
...
```bash
uv run discover-data --intent '{"entity_type":"table","constraints":[...]}' --limit 10
```
...
```bash
uv run discover-data "customer complaints" --limit 10
```
